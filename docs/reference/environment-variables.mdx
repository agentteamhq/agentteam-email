...
<ParamField path="AT_EMAIL_ADMIN_WEB_SERVER_DEBUG" type="string">
  Enables the web server's safe `app:frontend` debug namespace when set to any non-empty value. The value is not passed through as a `DEBUG` pattern, so wildcards such as `*` or `app:*` are ignored.
