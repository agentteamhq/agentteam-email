...
The default `compose.yaml` stack provides MongoDB, Redis, WildDuck, Haraka, ZoneMTA, and internal service URLs on the Compose network. Set the defaulted variables below only when you are changing that topology, using `compose.no-db.yaml`, or enabling an optional integration.
...
<ParamField path="AT_EMAIL_ADMIN_UID" type="number">
  Host user ID used for containers that set a runtime user.
...
<ParamField path="AT_EMAIL_ADMIN_GID" type="number">
  Host group ID used for containers that set a runtime user.
...
AgentTeam Email requires three MongoDB databases and one Redis instance. The default `compose.yaml` stack supplies them with bundled services. Set these variables only when using `compose.no-db.yaml` or an equivalent external data-service deployment.
