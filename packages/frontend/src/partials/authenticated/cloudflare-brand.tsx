...
export function CloudflareConnectButton({
  ...
  return (
    <Button
      className={cn(
        `w-full bg-[#f48120] font-semibold text-white shadow-xs hover:bg-[#de6f0b]
        focus-visible:ring-[#f48120]/40`,
        className
      )}
      ...
      type='button'
    >
      {busy ? <Spinner data-icon='inline-start' /> : <CloudflareGlyph data-icon='inline-start' />}
      {children}
    </Button>
  )
}
...
export function CloudflareLogo({ className }: { className?: string }) {
  return (
    ...
    >
      <CloudflareLogoPaths includeWordmark />
    </svg>
  )
}

function CloudflareGlyph({ 'data-icon': dataIcon }: { 'data-icon'?: string }) {
  return (
    <svg
      aria-hidden='true'
      data-icon={dataIcon}
      ...
      xmlns='http://www.w3.org/2000/svg'
    >
      <CloudflareLogoPaths monochrome />
    </svg>
  )
}

function CloudflareLogoPaths({
  ...
  const cloudBaseFill = monochrome ? 'currentColor' : '#fff'
  const cloudPrimaryFill = monochrome ? 'currentColor' : '#f48120'
  const cloudSecondaryFill = monochrome ? 'currentColor' : '#faad3f'
  const wordmarkFill = monochrome ? 'currentColor' : '#404041'

  return (
    <>
      <path
        fill={cloudBaseFill}
        d='M94.7,10.6,89.1,9.3l-1-.4-25.7.2V21.5l32.3.1Z'
      />
      <path
        fill={cloudPrimaryFill}
