...
describe('agent Access service', () => {
  ...
  it('rejects Paperclip OAuth client registration before Better Auth calls when OAuth connection management is not authorized', async () => {
    expect.hasAssertions()
    ...
    const { connectPaperclipAgentAccessForWeb } = await import('./service')
    ...
    await expect(
      ...
      message: 'OAuth connection management is not authorized',
      status: 403
    })
    expect(agentAccessTestState.adminCreateOAuthClient).not.toHaveBeenCalled()
    expect(agentAccessTestState.oauthClientFind).not.toHaveBeenCalled()
  })

  it('does not allow agent management authority to register a Paperclip OAuth client', async () => {
    expect.hasAssertions()
        ...
        Promise.resolve([
          {
            ...
            principalId: '01960000-0000-7000-8000-0000000000bb',
  ...
  it('allows explicit OAuth connection authority to register a Paperclip OAuth client', async () => {
    expect.hasAssertions()
    ...
    expect(agentAccessTestState.adminCreateOAuthClient).toHaveBeenCalledOnce()
  })

  it('validates malformed Paperclip OAuth client registration before Agent Access management authorization', async () => {
    expect.hasAssertions()
    ...
    await expect(
      ...
      message: 'Invalid Agent Access request',
      status: 400
    })
    expect(agentAccessTestState.adminCreateOAuthClient).not.toHaveBeenCalled()
    expect(agentAccessTestState.oauthClientFind).not.toHaveBeenCalled()
  ...
  it('creates a Paperclip OAuth grant principal without exposing the client secret', async () => {
    expect.hasAssertions()

    const headers = new Headers({ cookie: 'session=abc' })
    const { connectPaperclipAgentAccessForWeb } = await import('./service')

    const result = await connectPaperclipAgentAccessForWeb({
      headers,
      input: {
        companyId: 'paperclip-company-1',
        pluginId: 'agentteam.paperclip-email-plugin'
    ...
    expect(agentAccessTestState.oauthClientFind).toHaveBeenCalledWith({
      referenceId: '01960000-0000-7000-8000-0000000000aa',
      softwareId: 'agentteam.paperclip-email-plugin'
    })
    expect(agentAccessTestState.adminCreateOAuthClient).toHaveBeenCalledWith({
      body: expect.objectContaining({
        client_name: 'Paperclip Email (paperclip-company-1)',
        ...
        software_id: 'agentteam.paperclip-email-plugin',
  ...
  it('reuses an existing Paperclip OAuth grant principal from backend-owned metadata', async () => {
    expect.hasAssertions()

    agentAccessTestState.oauthClientFind.mockReturnValue({
      exec: () =>
        Promise.resolve([
          {
            ...
            name: 'Other Paperclip client',
            referenceId: '01960000-0000-7000-8000-0000000000aa',
            softwareId: 'agentteam.paperclip-email-plugin'
          },
          {
            clientId: 'paperclip-client-existing',
            disabled: true,
            metadata: JSON.stringify({
              agentteamEmail: {
                companyId: 'paperclip-company-1',
            ...
            name: 'Existing Paperclip client',
            referenceId: '01960000-0000-7000-8000-0000000000aa',
            softwareId: 'agentteam.paperclip-email-plugin'
          }
        ])
  ...
  it('lists persisted Paperclip OAuth connections in the Agent Access view without secret-bearing fields', async () => {
    expect.hasAssertions()
        ...
        Promise.resolve([
          {
            ...
            clientSecret: 'secret-oauth-client-z',
            disabled: true,
            metadata: {
              agentteamEmail: {
          ...
          {
            clientId: 'paperclip-client-invalid',
            clientSecret: 'secret-oauth-client-invalid',
            disabled: false,
            metadata: {
              agentteamEmail: {
                companyId: 'paperclip-company-invalid',
                integration: 'paperclip',
                pluginId: 'other-plugin'
              }
            },
            name: 'Invalid Paperclip',
            referenceId: '01960000-0000-7000-8000-0000000000aa',
            softwareId: 'agentteam.paperclip-email-plugin'
          },
          {
            clientId: 'paperclip-client-a',
            clientSecret: 'secret-oauth-client-a',
            disabled: false,
            metadata: JSON.stringify({
              agentteamEmail: {
    ...
    expect(view.paperclipConnections).toStrictEqual([
      {
        clientId: 'paperclip-client-a',
        companyId: 'paperclip-company-a',
        name: 'Alpha Paperclip',
    ...
    expect(JSON.stringify(view)).not.toContain('secret-oauth-client')
    expect(agentAccessTestState.oauthClientFind).toHaveBeenCalledWith(
      {
        referenceId: '01960000-0000-7000-8000-0000000000aa',
        softwareId: 'agentteam.paperclip-email-plugin'
